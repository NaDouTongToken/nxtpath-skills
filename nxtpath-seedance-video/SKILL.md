...
| `--ref-image URL` | Repeatable. Public `http(s)` URL only. Local file paths and `data:` URLs are rejected — a public URL is required. Same price as text-to-video |
...
- 本地 `--ref-image` 文件路径会被拒绝：不会自动缩图，也不会转成 `data:` URL 提交。ark 线参考素材目前只收公网 http(s) URL；把图片放到可公网访问的地址再传 URL。网关字节通道（router #2186）落地后本 skill 会跟进。
